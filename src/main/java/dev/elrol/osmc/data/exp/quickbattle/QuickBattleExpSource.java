package dev.elrol.osmc.data.exp.quickbattle;
...
public class QuickBattleExpSource extends ExpSource {

    public static final MapCodec<QuickBattleExpSource> CODEC = RecordCodecBuilder.mapCodec(instance -> ExpSource.getCommonCodec(instance)
            .and(Codec.STRING.fieldOf("expFormula").forGetter(QuickBattleExpSource::getExpFormula))
            .and(OSMCConstants.TARGET_SPECIES_CODEC.listOf().fieldOf("targets").forGetter(QuickBattleExpSource::getTargets)
    ).apply(instance, (expGain, expFormula, targets) -> {
        QuickBattleExpSource data = new QuickBattleExpSource(expGain);
    ...
    public QuickBattleExpSource(int expGain) {
        super(expGain);
    }
    ...
    public ExpSourceType<?> getType() {
        return OSMCExpSourceTypeRegistry.COBBLEMON_QUICK_BATTLE_EXP_SOURCE;
    }
