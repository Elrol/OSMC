...
public class BlockHarvestExpSource extends ExpSource {

    public static final MapCodec<BlockHarvestExpSource> CODEC = RecordCodecBuilder.mapCodec(instance -> ExpSource.getCommonCodec(instance)
            .and(OSMCConstants.TARGET_BLOCK_CODEC.listOf().fieldOf("targets").forGetter(BlockHarvestExpSource::getTargets))
            .and(Codec.unboundedMap(Codec.STRING, Codec.STRING).fieldOf("requiredProperties").forGetter(BlockHarvestExpSource::getRequiredProperties)
    ).apply(instance, (expGain, targets, requiredProperties) -> {
                BlockHarvestExpSource data = new BlockHarvestExpSource(expGain);
    ...
    public BlockHarvestExpSource(int expGain) {
        super(expGain);
    }
    ...
    public ExpSourceType<?> getType() {
        return OSMCExpSourceTypeRegistry.COBBLEMON_HARVEST_EXP_SOURCE;
    }
    ...
    public List<SkillTrigger> getTriggers() {
        return List.of(SkillTrigger.HARVEST);
    }
}
