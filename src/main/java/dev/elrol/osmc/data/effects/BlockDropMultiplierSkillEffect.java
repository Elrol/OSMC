...
public class BlockDropMultiplierSkillEffect extends SkillEffect {

    public static final MapCodec<BlockDropMultiplierSkillEffect> CODEC = RecordCodecBuilder.mapCodec(instance -> SkillEffect.getCommonCodec(instance)
            .and(Codec.STRING.fieldOf("chanceFormula").forGetter(BlockDropMultiplierSkillEffect::getChanceFormula))
            .and(OSMCConstants.TARGET_ITEM_CODEC.listOf().fieldOf("targets").forGetter(BlockDropMultiplierSkillEffect::getTargets))
            .and(OSMCConstants.TARGET_BLOCK_CODEC.listOf().fieldOf("blocks").forGetter(BlockDropMultiplierSkillEffect::getBlocks)
    ).apply(instance, (reqLevel, expGainFormula, chanceFormula, targets, blocks) -> {
        BlockDropMultiplierSkillEffect data = new BlockDropMultiplierSkillEffect(reqLevel, expGainFormula, chanceFormula);
    ...
    public BlockDropMultiplierSkillEffect(int reqLevel, String expGainFormula, String chanceFormula) {
    ...
    public void addBlock(Block target) {
        Registries.BLOCK.getKey(target).ifPresent(key -> blocks.add(Either.left(key)));
    }
    ...
    public SkillEffectType<?> getType() {
        return OSMCSkillEffectTypeRegistry.BLOCK_DROP_MULTIPLIER_SKILL_EFFECT;
    }
